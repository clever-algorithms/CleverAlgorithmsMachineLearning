...
def pretty_print_code_listing(code_listing_line)
  # get the file
	bracket_part = code_listing_line[code_listing_line.rindex("]")..-1]
  filename = get_data_in_brackets(bracket_part)
  # get the caption
...
def build_introduction_chapter(bib, name="introduction")
  dirname = OUTPUT_DIR + "/"+name
  create_directory(dirname)
  source = "../book/c_"+name
  # process chapter overview
  parent = process_chapter_overview(name, bib, source, "Introduction") 
  # process all algorithms for algorithm chapter  
  Dir.entries(source).each do |file|
    next if file == "." or file == ".."
    next if File.extname(file) != ".tex"
...
def create_toc_html(algorithms, algorithms_completed, frontmatter)
  ...
  begin
    ...
    data = general_process_file(lines)  
		add_line(s, "<li><a href='introduction.html'>#{data.last[:chapter]}</a></li>")
    algos = collect_subpages_for_page2(data)
    add_line(s, "<ol>")
    algos.each do |filename|
      # super lazy at getting names - consider a better process
      algo_name = get_algorithm_name("../book/c_introduction/"+filename+".tex")
      add_line(s, "<li><a href='introduction/#{filename}.html'>#{algo_name}</a></li>")
    end
    add_line(s, "</ol>")
    add_line(s, "</ol>")
  end
  ...
  algorithms.each do |name|
    lines = get_all_data_lines("../book/c_#{name}.tex")
    data = general_process_file(lines)
		# can we link this lot?
		if algorithms_completed.include?(name)
    	add_line(s, "<li><a href='#{name}.html'>#{data.last[:chapter]}</a></li>")
		else
			add_line(s, "<li>#{data.last[:chapter]}</li>")
		end
    algos = collect_subpages_for_page(data)
    add_line(s, "<ol>")
    algos.each do |filename|
  ...
  # appendix
  add_line(s, "<li>Appendix A - R: Quick-Start Guide</li>")
  add_line(s, "<ol>")
  ...
  appendix.last[:content].each do |element|
    ...
    add_line(s, "<li>#{element[:section]}</li>")
  end
  add_line(s, "</ol>")  
  # errata
  add_line(s, "<li>Errata</li>")
  add_line(s, "</ol>")
  return s
end

def build_toc(algorithms, algorithms_completed, frontmatter)
  html = create_toc_html(algorithms, algorithms_completed, frontmatter)
...
# these are ordered
ALGORITHM_CHAPTERS = ["optimization", "regression", "regularization", "discriminant", "trees", "kernel", "bayesian", "lazy", "ensembles", "clustering", "dimension"]
ALGORITHM_CHAPTERS_COMPLETED = ["optimization", "regression"]
...
if __FILE__ == $0
  ...
  ALGORITHM_CHAPTERS_COMPLETED.each {|name| build_algorithm_chapter(name, bib) }
  # advaced topics 
