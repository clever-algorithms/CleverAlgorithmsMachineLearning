...
class Float
  def round_to(x)
  	return self if self.nan? or self.inf?
...
def generate_results
  ...
  if File.exists?(ALGORITHM_RESULTS) 
    ...
    raw.each { |line| result_algorithm_names_list << line.split(',')[0].strip.downcase}
  end
  
  # process algorithms
  results = ""
  algorithms_list.each_with_index do |line, i|
    algorithm_name, scores = line.split(',')[0].strip, nil
...
def normalize_results
  ...
  normalized_scores = Array.new(algorithms_list[0].length-2) { [100000000.0, 0.0] } 
  # calculate min/max
  algorithms_list.each do |row|      
    row[2..row.length-1].each_with_index do |v, i|
      normalized_scores[i][0] = v.to_f if v.to_f < normalized_scores[i][0].to_f
      normalized_scores[i][1] = v.to_f if v.to_f > normalized_scores[i][1].to_f
    end
  end
  puts normalized_scores.inspect
  # normalize scores
  results = ""
  algorithms_list.each do |row|
    sum = 0.0
    row[2..row.length-1].each_with_index do |v, i|      
      # (v-min)/(max-min) 
      range = (normalized_scores[i][1] - normalized_scores[i][0])
      if range != 0.0      
      	sum += (v.to_f - normalized_scores[i][0]) / range
      end
    end
    # calculate rank
    results << "#{row.join(",").strip},#{sum}\n"
  end  
...
def generate_organized_results  
  ...
  algorithms_list.each do |row|
    ...
    group = row[1].strip.downcase
    data[group] = [] if !data.has_key?(group)
    data[group] << [row[0]] + row[2..row.length-1]
  end
  # organize    
  results = ""
